...
class TemperatureSensor(MeasurementSensor):
    # Returns True when the temperature is at or below freezing (0 C).
    def is_freezing(self) -> bool:
        return self.value <= 0


# PressureSensor specializes MeasurementSensor for pressure readings.
# It tracks a maximum safe pressure and can report if the value is unsafe.
class PressureSensor(MeasurementSensor):
    ...
    # Returns True when the current pressure exceeds the safe limit.
